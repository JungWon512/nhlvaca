	...
	*----------------------------------------------------------------------------*
	...
	$(document).ready(function() {
		$("#hiddenBody").show();
		$("#hiddenBody").find("input").attr("type", "text");
	...
	* 3. 출 력 변 수 : N/A
	...
	/*------------------------------------------------------------------------------
		...
		* 3. 출 력 변 수 : N/A
		------------------------------------------------------------------------------*/
	function fn_Init() {
		MessagePopup('YESNO',"등록한 내용이 초기화됩니다.<br/>초기화 하시겠습니까?", function(res){
			if(res){
		...
		MessagePopup('YESNO', saveMessage, function(res) {
			...
			if(res) {
				...
				if(fn_isNull(feeList)) {
					...
					srchData["grd_MhFee"] 		= feeImpsList;
	...
	function fn_InitSet() {
		...
		if(mv_RunMode == 1) {
			...
			$("#rc_dt").datepicker().datepicker("setDate", fn_getToday());			// 접수일자
			$("#sel_sts_dsc").val("11");											// 진행상태
	...
	function fn_SetInfo() {
		...
		const results = sendAjax(data, "/LALM1004_selList", "POST");
		let result;
	...
	//**************************************
	function fn_MacoFee() {
		var resultsMacoFee = sendAjaxFrm("frm_MhSogCow", "/LALM1004_selMacoFee", "POST");
		var resultMacoFee;
