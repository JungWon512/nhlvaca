...
function fn_setEvent() {
	 ...
	 * 비고 kg별 단가 셋팅 
	 *****************************/
	function setKgAmt(lowsSbidLmtAm) {
		
		let rmkAmt = parseInt(lowsSbidLmtAm) / parseInt($("#cow_sog_wt").val());
		
		rmkAmt = parseInt(rmkAmt).toString();
	...
	$("#cow_sog_wt").on("change keyup paste", function(e) {
		...
		let lowsSbidLmtAm = "0";
			...
			if(!fn_isNull($("#lows_sbid_lmt_am_ex").val())) {
				...
				// rmkTxt = "kg당 " + setKgAmt(lowsSbidLmtAm) + " 원";
				
			}
		}
		
		// KG 단위 경매가 아닐 경우에만 비고 셋팅
		//if(parent.envList[0].GT_AUC_UPR_DSC != '1') {
		//	$("#rmk_cntn").val(rmkTxt);
		//}
	});
	 ...
	 * 경매번호 체크박스 이벤트
	 ******************************/
	$("#auto_inc_yn").on("input change", function(){				
		if($("input:checkbox[id=auto_inc_yn]").is(":checked")) {
			$("#auc_prg_sq").attr("disabled", true);
			$("#auc_prg_sq").val("");
		} else {
			$("#auc_prg_sq").attr("disabled", false);
			$("#auc_prg_sq").focus();
		}
	});
}
...
function fn_CheckRequiredVal() {
	...
	// 자동증가 경매번호 체크
	if(!$("#auto_inc_yn").is(":checked") && fn_isNull($.trim($("#auc_prg_sq").val()))) {
		MessagePopup("OK", "경매번호를 입력해주세요.", function(){$("#auc_prg_sq").focus();});
		return false;
	}
...
function fn_Save() {
	...
	let sogWt = $("#cow_sog_wt").val();
	let sbidLmtAm = $("#lows_sbid_lmt_am_ex").val();
	
	// 중량 입력 체크
	//if(sogWt === '0' || sogWt === '') {
					
	//	MessagePopup('OK','중량을 0이나 공백으로 입력 할 수 없습니다.');
	//	$("#cow_sog_wt").focus();
	//	return;
	//}
	
	// 예정가 입력 체크
	//if(sbidLmtAm === 0 || sbidLmtAm === '') {
					
	//	MessagePopup('OK','예정가를 0이나 공백으로 입력 할 수 없습니다.');
	//	$("#lows_sbid_lmt_am_ex").focus();
	//	return;
	//}
