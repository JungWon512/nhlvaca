...
<script type="text/javascript">
	...
	function fn_Search() {
		...
		if ($("#sh_sra_indv_amnno").val().length == 4) {
			...
			$("#sh_condition").val("9");
				...
				&& $("#sh_sra_indv_amnno").val().length > 4) {
			$("#sh_condition").val("5");
		}
		...
		//개체조회
		var results = sendAjaxFrm("frm_Search", "/LALM1001_selList", "POST");
		var result;
	...
	function fn_Save() {
		...
		if (!$("#sra_indv_amnno").is(":disabled")) {
			MessagePopup('YESNO', "신규등록하시겠습니까?", function(res) {
				if (res) {
					var results = sendAjaxFrm("frm_MhSogCow", "/LALM1001_insIndv", "POST");
					if (results.status != RETURN_SUCCESS) {
						showErrorMessage(results);
						return;
					} else {
			...
			MessagePopup('YESNO', "수정하시겠습니까?", function(res) {
				if (res) {
					var results = sendAjaxFrm("frm_MhSogCow", "/LALM1001_updIndv", "POST");
					if (results.status != RETURN_SUCCESS) {
						showErrorMessage(results);
						return;
					} else {
	...
	function fn_Print() {
		...
		ReportPopup('LALM1001R', TitleData, 'grd_MmIndv', 'V');
	}
	...
	function fn_Delete() {
		...
		MessagePopup('YESNO', "삭제하시겠습니까?", function(res) {
			if (res) {
				var results = sendAjaxFrm("frm_MhSogCow", "/LALM1001_delIndv",
						"POST");
				if (results.status != RETURN_SUCCESS) {
					showErrorMessage(results);
					return;
	...
	function fn_CreateGrd_MmIndv(data) {
		...
		var searchResultColNames = [ "경제통합사업장코드", "귀표번호", "축산축종구분코드", "농가식별번호",
				"농가식별", "농장관리번호", "농가명", "생년월일", "어미구분", "KPN번호", "성별",
				"어미귀표번호", "어미산차", "계대", "개체식별번호", "등록번호", "신규여부", "등록구분",
		...
		var searchResultColModel = [ {
			...
			name : "SRA_INDV_AMNNO",
			index : "SRA_INDV_AMNNO",
			width : 40,
			align : 'center',
			formatter : 'gridIndvFormat'
		}, {
			name : "SRA_SRS_DSC",
			index : "SRA_SRS_DSC",
			width : 40,
			...
			formatter : "select",
			editoptions : {
				value : fn_setCodeString("SRA_INDV_BRDSRA_RG_DSC", 1)
			}
		}, {
			name : "KPN_NO",
			index : "KPN_NO",
			...
			formatter : "select",
			editoptions : {
				value : fn_setCodeString("INDV_SEX_C", 1)
			}
		}, {
			name : "MCOW_SRA_INDV_AMNNO",
			index : "MCOW_SRA_INDV_AMNNO",
			width : 40,
			align : 'center',
			formatter : 'gridIndvFormat'
		}, {
			...
			name : "SRA_INDV_PASG_QCN",
			index : "SRA_INDV_PASG_QCN",
			width : 40,
			align : 'right'
		}, {
			name : "INDV_ID_NO",
			...
			name : "SRA_INDV_BRDSRA_RG_NO",
			index : "SRA_INDV_BRDSRA_RG_NO",
			width : 40,
			align : 'center'
		}, {
			name : "ANW_YN",
	...
	function fn_setFrm_Mmlndv(sel_data) {
		...
		var results = sendAjax(srchData, "/LALM1001_selIndvDetail", "POST");
		var result;
		if (results.status != RETURN_SUCCESS) {
	...
	function fn_BtnInit() {
		...
		parent.layerPopupPage(pgid, menu_id, null, null, 800, 600, function(result) {
			...
			$("#grfa_sra_kpn_no").val(result.GRFA_SRA_KPN_NO);
			$("#grfa_sra_indv_eart_no").val(result.GRFA_SRA_INDV_EART_NO);
			$("#grmo_sra_indv_eart_no").val(result.GRMO_SRA_INDV_EART_NO);
			$("#mtgrfa_sra_kpn_no").val(result.MTGRFA_SRA_KPN_NO);
			$("#mtgrfa_sra_indv_eart_no").val(result.MTGRFA_SRA_INDV_EART_NO);
			$("#mtgrmo_sra_indv_eart_no").val(result.MTGRMO_SRA_INDV_EART_NO);
			$("#sra_fhs_birth").val(result.SRA_FHS_BIRTH);
			$("#metrb_metqlt_grd").val(result.METRB_METQLT_GRD);
		});
...
<body>
	<div class="contents">
		...
		<section class="content">
						...
						<table>
							...
							<tbody>
								<tr>
									...
									<td><input type="text" id="sh_sra_indv_amnno"
										class="popup" maxlength="9"></td>
									<td></td>
									<th scope="row">농가명</th>
									<td><input type="text" id="sh_ftsnm"></td>
									<td></td>
								</tr>
							</tbody>
						</table>
					...
					<form id="frm_MhSogCow">
						...
						<table>
							...
							<tbody>
								<tr>
									<th>귀표번호</th>
									<td><input class="popup" type="text" id="sra_indv_amnno" maxlength="15" /></td>
									<th>생년월일</th>
									<td>
										<div class="cellBox">
											<div class="cell">
												<input type="text" class="date popup" id="birth" name="birth" maxlength="10">
											</div>
										</div>
									</td>
									<th>어미구분</th>
									<td><select id="mcow_dsc">
									</select></td>
									<th>농가정보</th>
									<td colspan="3">
										<div class="cellBox v_addr">
											...
											<div class="cell pl2"
												style="width: 20px; text-align: center;">-</div>
								...
								<tr>
									...
									<th>어미귀표번호</th>
									<td><input type="text" id="mcow_sra_indv_amnno" /></td>
									<th>산차</th>
									<td><input type="text" id="matime" /></td>
									<th>개체식별번호</th>
									<td><input type="text" id="indv_id_no" /></td>
									<th>등록번호</th>
									<td><input type="text" id="sra_indv_brdsra_rg_no" /></td>
								</tr>
								<tr>
									<th>계대</th>
									<td><input type="text" id="sra_indv_pasg_qcn" /></td>
									<th>KPN번호</th>
									<td><input type="text" id="kpn_no" /></td>
									<th>등록구분</th>
									<td><select id="rg_dsc"></select></td>
									<th>축종구분</th>
									<td>
								...
								<tr style="display:none;">
									<th>기타정보</th>
									<td colspan="9">
										<input type="text" id="grfa_sra_kpn_no" />
										<input type="text" id="grfa_sra_indv_eart_no" />
										<input type="text" id="grmo_sra_indv_eart_no" />
										<input type="text" id="mtgrfa_sra_kpn_no" />
										<input type="text" id="mtgrfa_sra_indv_eart_no" />
										<input type="text" id="mtgrmo_sra_indv_eart_no" />
										<input type="text" id="sra_fhs_birth" />
										<input type="text" id="metrb_metqlt_grd" />
									</td>
								</tr>
							</tbody>
						</table>
			...
			<div class="listTable rsp_v"><table id="grd_MmIndv"></table>
				<!-- 페이징 -->
				<div id="pager"></div>
			</div>
		</section>
