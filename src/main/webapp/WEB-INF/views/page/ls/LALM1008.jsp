            ...
            if((parseInt($("#sbt_am").val()) == 0 && $("#sbt_pmr").attr('disabled')) || (parseInt($("#sbt_pmr").val()) == 0 && $("#sbt_am").attr('disabled'))) {
            	MessagePopup('YESNO',"예정가를 최초 예정가로 변경합니다. 변경하시겠습니까?",function(res){
                    if(res){
                        ...
                        result = sendAjax(insDataObj, "/LALM1008_updFirstBatPrice", "POST");
            	...
            	MessagePopup('YESNO',"예정가를 변경하시겠습니까?",function(res){
                    if(res){
                        var result        = null;
                        ...
                        result = sendAjax(insDataObj, "/LALM1008_updBatPrice", "POST");
...
//         $( "#sbt_pmr_text" ).hide();
    ...
    function fn_Search(){                
        ...
        if(resultsAucQcnList.status != RETURN_SUCCESS) {
            ...
            if(resultAucQcnList[0]["DDL_YN"] == "1") {
            	...
            	var results = sendAjaxFrm("frm_Search", "/LALM1008_selList", "POST");        
                var result;
    ...
    function fn_Save(){
         ...
         MessagePopup('YESNO',"저장 하시겠습니까?",function(res){
             if(res){
                 ...
                 result = sendAjax(insDataObj, "/LALM1008_updConti", "POST");
    ...
    function fn_CreateGrid(data){              
        ...
        var searchResultColNames = ["","H사업장코드","H경매일자","H원표번호","H판매상태구분","H거래인","H참여자번호","H혈통금액","H출하수수료수기적용여부","H출하수수료수기등록","H판매수수료수기적용여부","H판매수수료수기등록"
						        	...
						        	,"경매번호", "경매대상구분", "출하주", "접수일", "성별", "최소 예정가", "예정가", "예정가<br>변경횟수", "중량", "경매참가번호", "", "낙찰단가", "낙찰금액","진행상태","변경사유", "응찰하안가ex", "마감차수", "구분", "낮출금액", ""];        
        ...
        $("#grd_MhSogCow").jqGrid({
            ...
            afterSaveCell: function(rowid, cellname, value, iRow, iCol) {
            	/* 20240613 JJW : 호가낙찰로인한 변경건을 위한 예외 제거
    ...
    function fn_popSearch(rowid, colkey,flag){
        ...
        //var pre_sra_mwmnnm = $("#grd_MhSogCow").jqGrid("getCell", rowid, 'SRA_MWMNNM');
                ...
                <div class="grayTable rsp_v">
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr>
                                <th scope="row">
                                    <input type="radio" id="am_rto_dsc_1" name="am_rto_dsc_radio" value="1" onclick="javascript:fn_setChgRadio('am_rto_dsc','1');fn_setChgRadioAmRtoDsc('1');"/> 금액(원)
                                </th>
                                <td>
